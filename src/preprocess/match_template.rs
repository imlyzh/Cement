...
pub fn match_template(record: &mut MatchRecord, temp: &Value, inp: &Value) -> Result<(), SyntaxMatchError> {
	match (temp.clone(), inp.clone()) {
		(Value::Sym(id), v) => {
			record.maps.get_mut().insert(id.clone(), v)
			.map_or(Err(SyntaxMatchError::RepeatedSymbol(id)), |_|Ok(()))?;
		},
		...
		(Value::List(a), Value::List(b)) => {
			...
			*a.clone().0.back().unwrap() == Value::Sym(Arc::new(Symbol::new("..."))) {
				...
				a.clone().0.iter().try_for_each(
					|x| if *x == Value::Sym(Arc::new(Symbol::new("..."))) {
				...
				let r = b_expand_lst.map(
					|x| -> Result<MatchRecord, SyntaxMatchError> {
